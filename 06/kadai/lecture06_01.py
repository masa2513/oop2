...
class GameButton(QPushButton):
    ...
    def __check_win(self):
        ...
        for pattern in patterns:
            if self.__check_a_line(pattern):
                return True  # 勝敗が決まったら終了
        
        return False  # 勝敗が決まっていない

    def __check_a_line(self, pattern : list[int]):
        """
        指定されたパターンの3マスが同じマークかを判定
        
        Args:
            pattern: チェックする3マスのインデックスリスト
    ...
    def __check_draw(self):
        """
        引き分けの判定（全てのマスが埋まっているか）
        
        Returns:
            bool: 引き分けの場合True
        """
        # 全てのマスが埋まっているかチェック
        all_filled = all(btn.text() != "" for btn in self.buttons)
